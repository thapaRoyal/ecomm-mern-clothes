...
const StripeCheckout = () => {
  ...
  return (
    <div
      className="p-4 shadow-lg bg-body productCard-container"
      style={{ backgroundColor: "#2c2c6c" }}
    >
      {!succeeded && (
        <div>
          {coupon && totalAfterDiscount !== undefined ? (
            <p className="alert bg-white text-success font-weight-bold">{`Coupon applied. Total after discount: Rs.${totalAfterDiscount}`}</p>
          ) : (
            <p className="alert bg-white text-danger font-weight-bold">
              No coupon applied
            </p>
          )}
        </div>
      ...
      >
        ...
        <p
          className={
            succeeded ? "result-message text-white" : "result-message hidden"
          }
        >
          Payment Successfull.{" "}
          <Link
            to="/user/history"
            className="font-weight-bold text-white text-decoration-underline"
          >
            View order
          </Link>
        </p>
